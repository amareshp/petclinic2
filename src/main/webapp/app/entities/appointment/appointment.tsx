...
export interface IAppointmentState extends IPaginationBaseState {
  search: string;
}
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={this.sort('apptTime')}>
                  Appt Time <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  Slot <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  Vet <FontAwesomeIcon icon="sort" />
                </th>
                <th>
            ...
            <tbody>
              {appointmentList.map((appointment, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <TextFormat type="date" value={appointment.apptTime} format={APP_LOCAL_DATE_FORMAT} />
                  </td>
                  <td>{appointment.slot ? <Link to={`slot/${appointment.slot.id}`}>{appointment.slot.startTime}</Link> : ''}</td>
                  <td>{appointment.vet ? <Link to={`vet/${appointment.vet.id}`}>{appointment.vet.name}</Link> : ''}</td>
                  <td>{appointment.pet ? <Link to={`pet/${appointment.pet.id}`}>{appointment.pet.name}</Link> : ''}</td>
